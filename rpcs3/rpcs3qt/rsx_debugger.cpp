	...
	, exit(false)
{
	...
	if (const auto render = rsx::get_current_renderer(); render &&
		render->iomap_table.get_ioaddr(render->ctrl->get.load()) + 1)
	{
	...
	{
		if (const auto render = rsx::get_current_renderer(); render &&
			render->iomap_table.get_ioaddr(render->ctrl->put.load()) + 1)
		{
...
void rsx_debugger::GetMemory()
{
	...
	for (int i = 0, addr = m_addr; i < item_count; i++, addr += 4)
	{
		...
		if (const u32 ea = rsx::get_current_renderer()->iomap_table.get_ioaddr(addr);
			ea + 1)
		{
...
#define DISASM(string, ...) { if(disasm.empty()) disasm = fmt::format((string), ##__VA_ARGS__); else disasm += (' ' + fmt::format((string), ##__VA_ARGS__)); }
	...
	{
		const auto args = vm::get_super_ptr<u32>(rsx::get_current_renderer()->iomap_table.get_ioaddr(ioAddr + 4));
