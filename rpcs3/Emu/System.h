...
struct cfg_root : cfg::node
{
	struct node_core : cfg::node
	{
		...
		cfg::_int<1, 4> spurs_urgent_queue_size{this, "SPURS Urgent queue size", 2}; // HACK. If less then 4, max number of queued urgent SPURS commands per JobChain
	...
	struct node_video : cfg::node
	{
		...
		cfg::_int<-100, 100> texture_lod_bias{this, "Texture LOD Bias", 0};
